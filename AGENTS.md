...
`Scripts/quality.sh` runs Swift formatting lint, `swift build`, `swift test --enable-code-coverage`, and enforces at least **80% line coverage** for unit-testable production code. It excludes test/build artifacts plus the hardware- and OS-boundary adapters `MultitouchBridge.swift`, console `main.swift` (`Sources/TouchpadWMSpikeConsole/main.swift`), `AccessibilityWindowService.swift`, `SwitcherGestureCoordinator.swift`, and `SwitcherOverlayPanel.swift`; retain their required real-hardware/real-Accessibility-permission acceptance test. Extract any pure logic out of these adapters (e.g. `AccessibilityWindowMetadata.swift`, `SwitcherGestureRecognizer.swift`, `SwitcherOverlayGeometry.swift`) into unit-tested files rather than excluding more than the adapter itself. It uses the selected Xcode toolchain's `llvm-cov` directly; do not substitute `xccov`.
