...
grep -F 'export ' "$QUALITY_LOG" | grep -F -- '-instr-profile ' | grep -F -- '--ignore-filename-regex ' | grep -F -- '/Tests/' | grep -F -- 'MultitouchBridge\.swift' | grep -F -- 'main\.swift' | grep -F -- 'TouchpadWMApp\.swift' | grep -F -- 'AccessibilityWindowService\.swift'
