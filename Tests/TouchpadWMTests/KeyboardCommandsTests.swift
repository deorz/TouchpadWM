...
final class KeyboardCommandsTests: XCTestCase {
  func testRightOptionWithArrowMapsFixedCommands() {
    ...
    XCTAssertEqual(interpreter.consume(.keyDown(keyCode: 126, shiftIsPressed: false)), .topRightQuarter)
    XCTAssertEqual(interpreter.consume(.keyDown(keyCode: 125, shiftIsPressed: false)), .bottomRightQuarter)
    XCTAssertEqual(interpreter.consume(.keyDown(keyCode: 123, shiftIsPressed: true)), .leftThreeQuarters)
  ...
  func testRouterBlocksUnavailablePermissionAndDispatchesAvailableCommand() {
    ...
    XCTAssertFalse(router.consume(.keyDown(keyCode: 123, shiftIsPressed: false), permission: .unavailable) { commands.append($0) })
    XCTAssertEqual(commands, [])

    XCTAssertTrue(router.consume(.keyDown(keyCode: 123, shiftIsPressed: false), permission: .available) { commands.append($0) })
